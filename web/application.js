  ...
  document.addEventListener('DOMContentLoaded', event => {
    ...
    function connect() {
      port.connect().then(() => {
        ...
        document.getElementById("receiver_lines").innerHTML = '';
        document.getElementById("sender_lines").innerHTML = '';
        document.getElementById("command_line").innerHTML = '';
        port.onReceive = data => {
          let textDecoder = new TextDecoder();
          let decodedData = textDecoder.decode(data);
          console.log("a", decodedData);
          if (decodedData && decodedData.indexOf('[CLEAR]') > -1) {
            currentReceiverLine = null;
            document.getElementById("receiver_lines").innerHTML = '';
            console.log("Clearing");
            decodedData = decodedData.replace('[CLEAR]', '');
          }
          if (data.getInt8() === 13) {
            currentReceiverLine = null;
          } else {
            appendLines('receiver_lines', decodedData);
          }
        };
